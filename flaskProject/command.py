...
# For all the Restful APIs, since the data we use is just one layer key-value pair with no outermost unique key, the implementation is not so standard
# Our implementation choose the first meaningful key(gap the '_id' which is unique key generated by MongoDB but with no actual meaning) EEID as the main key, acting as if it's outermost in PUT, PATCH, POST, DELETE
# This may be not so standard for Restful APIs, but in this way we can balance the meaningfulness of the function and the implementation of the basic norm of the Restful APIs
